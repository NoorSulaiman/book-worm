...
class ProgressForm extends React.Component {
  ...
  validate = data => {
    ...
    if (data > this.props.book.pages) {
      errors.progress = "Progress can't be over 100%";
    }
    if (data < 1) {
      errors.progress = "Progress can't be less than 1";
    }
    return errors;
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Button primary disabled={this.state.disabled}>
          Submit
        </Button>
      </Form>
    );
